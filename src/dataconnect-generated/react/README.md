...
```javascript
useGetPostsByAuthor(dc: DataConnect, vars: GetPostsByAuthorVariables, options?: useDataConnectQueryOptions<GetPostsByAuthorData>): UseDataConnectQueryResult<GetPostsByAuthorData, GetPostsByAuthorVariables>;
```
You can also pass in a `DataConnect` instance to the Query hook function.
```javascript
useGetPostsByAuthor(vars: GetPostsByAuthorVariables, options?: useDataConnectQueryOptions<GetPostsByAuthorData>): UseDataConnectQueryResult<GetPostsByAuthorData, GetPostsByAuthorVariables>;
```
...
```javascript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, GetPostsByAuthorVariables } from '@dataconnect/generated';
import { useGetPostsByAuthor } from '@dataconnect/generated/react'

export default function GetPostsByAuthorComponent() {
  // The `useGetPostsByAuthor` Query hook requires an argument of type `GetPostsByAuthorVariables`:
  ...
  const query = useGetPostsByAuthor(getPostsByAuthorVars);
  // Variables can be defined inline as well.
  const query = useGetPostsByAuthor({ authorId: ..., });

  // You can also pass in a `DataConnect` instance to the Query hook function.
  const dataConnect = getDataConnect(connectorConfig);
  const query = useGetPostsByAuthor(dataConnect, getPostsByAuthorVars);

  // You can also pass in a `useDataConnectQueryOptions` object to the Query hook function.
  const options = { staleTime: 5 * 1000 };
  const query = useGetPostsByAuthor(getPostsByAuthorVars, options);

  // You can also pass both a `DataConnect` instance and a `useDataConnectQueryOptions` object.
  const dataConnect = getDataConnect(connectorConfig);
  const options = { staleTime: 5 * 1000 };
  const query = useGetPostsByAuthor(dataConnect, getPostsByAuthorVars, options);
...
You can execute the `GetFriendshipsForUser` Query using the following Query hook function, which is defined in [dataconnect-generated/react/index.d.ts](./index.d.ts):

```javascript
useGetFriendshipsForUser(dc: DataConnect, options?: useDataConnectQueryOptions<GetFriendshipsForUserData>): UseDataConnectQueryResult<GetFriendshipsForUserData, undefined>;
```
You can also pass in a `DataConnect` instance to the Query hook function.
```javascript
useGetFriendshipsForUser(options?: useDataConnectQueryOptions<GetFriendshipsForUserData>): UseDataConnectQueryResult<GetFriendshipsForUserData, undefined>;
```

### Variables
The `GetFriendshipsForUser` Query has no variables.
### Return Type
Recall that calling the `GetFriendshipsForUser` Query hook function returns a `UseQueryResult` object. This object holds the state of your Query, including whether the Query is loading, has completed, or has succeeded/failed, and any data returned by the Query, among other things.

To check the status of a Query, use the `UseQueryResult.status` field. You can also check for pending / success / error status using the `UseQueryResult.isPending`, `UseQueryResult.isSuccess`, and `UseQueryResult.isError` fields.

To access the data returned by a Query, use the `UseQueryResult.data` field. The data for the `GetFriendshipsForUser` Query is of type `GetFriendshipsForUserData`, which is defined in [dataconnect-generated/index.d.ts](../index.d.ts). It has the following fields:
```javascript
...
### Using `GetFriendshipsForUser`'s Query hook function

```javascript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig } from '@dataconnect/generated';
import { useGetFriendshipsForUser } from '@dataconnect/generated/react'

export default function GetFriendshipsForUserComponent() {
  // You don't have to do anything to "execute" the Query.
  // Call the Query hook function to get a `UseQueryResult` object which holds the state of your Query.
  const query = useGetFriendshipsForUser();

  // You can also pass in a `DataConnect` instance to the Query hook function.
  const dataConnect = getDataConnect(connectorConfig);
  const query = useGetFriendshipsForUser(dataConnect);

  // You can also pass in a `useDataConnectQueryOptions` object to the Query hook function.
  const options = { staleTime: 5 * 1000 };
  const query = useGetFriendshipsForUser(options);

  // You can also pass both a `DataConnect` instance and a `useDataConnectQueryOptions` object.
  const dataConnect = getDataConnect(connectorConfig);
  const options = { staleTime: 5 * 1000 };
  const query = useGetFriendshipsForUser(dataConnect, options);
...
export default function CreateUserComponent() {
  ...
  // Variables can be defined inline as well.
  mutation.mutate({ username: ..., email: ..., passwordHash: ..., role: ..., });
