...
- [**Queries**](#queries)
  ...
  - [*GetFriendshipsForUser*](#getfriendshipsforuser)
- [**Mutations**](#mutations)
...
```typescript
getPostsByAuthor(vars: GetPostsByAuthorVariables): QueryPromise<GetPostsByAuthorData, GetPostsByAuthorVariables>;
...
```typescript
getPostsByAuthor(dc: DataConnect, vars: GetPostsByAuthorVariables): QueryPromise<GetPostsByAuthorData, GetPostsByAuthorVariables>;
...
### Variables
The `GetPostsByAuthor` query requires an argument of type `GetPostsByAuthorVariables`, which is defined in [dataconnect-generated/index.d.ts](./index.d.ts). It has the following fields:
...
```typescript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, getPostsByAuthor, GetPostsByAuthorVariables } from '@dataconnect/generated';

// The `GetPostsByAuthor` query requires an argument of type `GetPostsByAuthorVariables`:
const getPostsByAuthorVars: GetPostsByAuthorVariables = {
  authorId: ..., 
};

// Call the `getPostsByAuthor()` function to execute the query.
// You can use the `await` keyword to wait for the promise to resolve.
const { data } = await getPostsByAuthor(getPostsByAuthorVars);
// Variables can be defined inline as well.
const { data } = await getPostsByAuthor({ authorId: ..., });

// You can also pass in a `DataConnect` instance to the action shortcut function.
const dataConnect = getDataConnect(connectorConfig);
const { data } = await getPostsByAuthor(dataConnect, getPostsByAuthorVars);

console.log(data.posts);

// Or, you can use the `Promise` API.
getPostsByAuthor(getPostsByAuthorVars).then((response) => {
...
import { connectorConfig, getPostsByAuthorRef, GetPostsByAuthorVariables } from '@dataconnect/generated';
...
const ref = getPostsByAuthorRef(getPostsByAuthorVars);
// Variables can be defined inline as well.
const ref = getPostsByAuthorRef({ authorId: ..., });

// You can also pass in a `DataConnect` instance to the `QueryRef` function.
const dataConnect = getDataConnect(connectorConfig);
const ref = getPostsByAuthorRef(dataConnect, getPostsByAuthorVars);
...
You can execute the `GetFriendshipsForUser` query using the following action shortcut function, or by calling `executeQuery()` after calling the following `QueryRef` function, both of which are defined in [dataconnect-generated/index.d.ts](./index.d.ts):
```typescript
getFriendshipsForUser(): QueryPromise<GetFriendshipsForUserData, undefined>;
...
You can also pass in a `DataConnect` instance to the action shortcut function or `QueryRef` function.
```typescript
getFriendshipsForUser(dc: DataConnect): QueryPromise<GetFriendshipsForUserData, undefined>;
...
If you need the name of the operation without creating a ref, you can retrieve the operation name by calling the `operationName` property on the getFriendshipsForUserRef:
```typescript
const name = getFriendshipsForUserRef.operationName;
console.log(name);
```

### Variables
The `GetFriendshipsForUser` query has no variables.
### Return Type
Recall that executing the `GetFriendshipsForUser` query returns a `QueryPromise` that resolves to an object with a `data` property.

The `data` property is an object of type `GetFriendshipsForUserData`, which is defined in [dataconnect-generated/index.d.ts](./index.d.ts). It has the following fields:
```typescript
...
```
### Using `GetFriendshipsForUser`'s action shortcut function

```typescript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, getFriendshipsForUser } from '@dataconnect/generated';


// Call the `getFriendshipsForUser()` function to execute the query.
// You can use the `await` keyword to wait for the promise to resolve.
const { data } = await getFriendshipsForUser();

// You can also pass in a `DataConnect` instance to the action shortcut function.
const dataConnect = getDataConnect(connectorConfig);
const { data } = await getFriendshipsForUser(dataConnect);

console.log(data.friendships);

// Or, you can use the `Promise` API.
getFriendshipsForUser().then((response) => {
  const data = response.data;
  console.log(data.friendships);
});
```

### Using `GetFriendshipsForUser`'s `QueryRef` function

```typescript
import { getDataConnect, executeQuery } from 'firebase/data-connect';
import { connectorConfig, getFriendshipsForUserRef } from '@dataconnect/generated';


// Call the `getFriendshipsForUserRef()` function to get a reference to the query.
const ref = getFriendshipsForUserRef();

// You can also pass in a `DataConnect` instance to the `QueryRef` function.
const dataConnect = getDataConnect(connectorConfig);
const ref = getFriendshipsForUserRef(dataConnect);
...
// Variables can be defined inline as well.
const { data } = await createUser({ username: ..., email: ..., passwordHash: ..., role: ..., });
...
// Variables can be defined inline as well.
const ref = createUserRef({ username: ..., email: ..., passwordHash: ..., role: ..., });
